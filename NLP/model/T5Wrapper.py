...
from NLP.utils.utils import round_nestedList
from NLP.model.generation import greedy_generation
...
class T5Wrapper(nn.Module):
    ...
    def predict_mulChoice(self, batch, length_normalization):
        ...
        return (
            predicted_choice.cpu().detach().numpy().tolist(),
            round_nestedList(score_ofChoices.cpu().detach().numpy().tolist(), 5),
            round_nestedList(logProbs_ofAllChoicesIds.cpu().detach().numpy().tolist(), 4),
            len_allChoices.cpu().detach().numpy().tolist(),
        )
