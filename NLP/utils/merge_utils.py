...
from NLP.utils.analysis_utils import *
...
BASIC_INFORMATION = {
    "bigscience/T0_3B": {
        ...
        "dump_dir": f"/temp",
        "load_dir": f"/scratch/shared/ia3_checkpoints/ia3",
    },
    "t5-base": {
        "base_model": "t5-base",
        "dump_dir": f"/temp",
        "load_dir": f"/mnt/nfs/shared/model_merging_finetuning_training/model_merging_exp_out/exp_out/training/t5-base",
    },
    "t5-large": {
        "base_model": "t5-large",
        "dump_dir": f"/temp",
        "load_dir": f"/mnt/nfs/shared/model_merging_finetuning_training/model_merging_exp_out/exp_out/training/t5-large",
    },
}
...
def load_t5(merging_tasks, model_key, pretrained_state_dict):
    ...
    # load the finetune and pretrained checkpoints
    try:
        filepaths = [os.path.join(load_dir, task, "best_model.pt") for task in merging_tasks]
        ft_checks = [torch.load(fp) for fp in filepaths]
    except FileNotFoundError:
        load_dir = load_dir.replace("/mnt/nfs", "/scratch")
        filepaths = [os.path.join(load_dir, task, "best_model.pt") for task in merging_tasks]
        ft_checks = [torch.load(fp) for fp in filepaths]
