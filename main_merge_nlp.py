...
from NLP.utils.merge_utils import load_ia3, vector_to_state_dict
...
def run(args):
    ...
    (
        tv_flat_checks, ### 2-D tensor, rows of task vectors
        flat_ptm, ### 1-D tensor, flattened pretrained task model
        flat_ft, ### 2-D tensor, flattened finetuned task models
        ft_checks, ### list of fine-tuned state_dicts
        ptm_check, ### pretrained state_dict
        remove_keys, ### list of keys to remove from the state_dict
    ) = load_ia3(datasets, server.config_toInit.pretrained_model)
...
def random_block_merge(args, server, task_vectors_all_layers, sparsified_task_vectors_all_layers,
    ...
    for t in range(T):
        ...
        dsu_blocks[block_max].union(i_max, j_max, to_tune) ### unify parameters
