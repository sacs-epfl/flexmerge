...
from beit3.glossary import normalize_word
from beit3.randaug import RandomAugment

MAX_VQAV2_TEST_SIZE = 20000
        ...
        for _index_file in index_files:
            ...
            with open(index_file, mode="r", encoding="utf-8") as reader:
                ...
                if subsample_size is not None:
                    ...
                    items_subset = rng.sample(items[offset:], min(subsample_size, len(items) - offset))
                    items = items[:offset] + items_subset
                    print("Uniformly subsampled %d image-text pairs from %s with seed 42." % (len(items_subset), index_file))
...
class ImageNetDataset(BaseDataset):
    ...
    def make_dataset_index(cls, train_data_path, val_data_path, index_path):
        ...
        data_path_prefix = os.path.dirname(val_data_path) + "/"
        classes, class_to_idx = cls._find_classes(val_data_path)
...
def create_dataset_by_split(args, split, is_train=True):
    ...
    if args.task == "vqav2" and MAX_VQAV2_TEST_SIZE > 0 and split == "test":
        opt_kwargs["subsample_size"] = MAX_VQAV2_TEST_SIZE
    ...
    # Reduces the size of the test set for VQAv2 to speed up testing
    # if args.task == "vqav2" and MAX_VQAV2_TEST_SIZE > 0 and split == "test":
    #     gen = torch.Generator(device="cpu")
    #     gen.manual_seed(42)
    #     perm = torch.randperm(len(dataset), generator=gen)
    #     indices = perm[:MAX_VQAV2_TEST_SIZE].tolist()
    #     dataset = Subset(dataset, indices)
    # print("Dataset %s with %d items!" % (dataset, len(dataset)))
