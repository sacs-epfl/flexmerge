...
from Data.my_datasets.templates import get_templates
from model.mtl_models import ClassificationHead, ImageEncoder
from Utils.variables_and_paths import TQDM_BAR_FORMAT

def build_classification_head(model: nn.Module, dataset_name: str, template: List[Callable[[str], str]],
                              data_location: str, device: torch.device) -> ClassificationHead:
    """
...
def get_classification_head(model, model_dir, datapath, dataset: str) -> nn.Module:
    """
    ...
    filename = os.path.join(model_dir, model, "heads", f"head_{dataset}.pt")
    if os.path.exists(filename):
        print(f"Loading classification head for {model} on {dataset} from {filename}")
    ...
    model = ImageEncoder(model, keep_lang=True).model
    template = get_templates(dataset)

    classification_head = build_classification_head(model, dataset, template, datapath, "cuda")
